...
5. **Dependency Inversion Principle & Bang-for-Buck Inversions** — core behavior constructs concrete services, reaches into global state, or couples directly to framework, network, or time APIs. Look for high-ROI opportunities where inverting control eliminates coupling or testing friction with minimal churn:
   - Callers passing parameters, options, or domain configs into a workflow/runner rather than the callee maintaining a growing conditional ladder of special cases (e.g. letting callers declare domain options directly instead of the runner hardcoding domain-specific bash/code switches).
   - Core domain or calculation logic that hardcodes environmental assumptions, side effects, or global thresholds instead of accepting them as parameters.
   - Tight coupling to global environment, clocks, filesystem, or network where an injected parameter or narrow port creates an instant testing seam.
   - Depend on a small meaningful boundary and inject it only when that improves testing, extension, or substitution — avoid speculative architecture or deep abstraction layers where a simple parameter inversion solves the problem.
