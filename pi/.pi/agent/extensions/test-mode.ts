...
const STREAM_CHUNKS = [
	`# Quuxifier Burst — Packet Bravo-Zulu-042
...
The audit produced a faint whistling sound and three contradictory  
maps, all of which were correct on alternate Thursdays.`,
...
export default function testModeExtension(pi: ExtensionAPI) {
	...
	pi.registerEntryRenderer<TestBurstData>("test-burst-chunk", (entry, { expanded }, theme) => {
		const data = entry.data ?? { timestamp: 0, chunkIndex: 0, chunkCount: STREAM_CHUNKS.length, text: "" };
		const box = new Box(1, 1, (text) => theme.bg("customMessageBg", text));
		box.addChild(
			new Text(
				`${theme.fg("accent", theme.bold("TEST BURST"))} ${theme.fg(
					"dim",
					`${data.chunkIndex + 1}/${data.chunkCount} · ${new Date(data.timestamp).toLocaleTimeString()}`,
				)}`,
				0,
				0,
			),
		);
		box.addChild(new Markdown(data.text, 0, 1, getMarkdownTheme()));
		if (expanded) {
			box.addChild(new Text(theme.fg("dim", "Test mode terminal-speed chunk"), 0, 1));
		}
		return box;
	});
	...
	pi.registerCommand("test", {
		description: "Toggle streaming terminal-speed gibberish mode",
	...
	pi.on("input", async (event) => {
		...
		for (const [chunkIndex, text] of STREAM_CHUNKS.entries()) {
			pi.appendEntry<TestBurstData>("test-burst-chunk", {
				timestamp,
				chunkIndex,
				chunkCount: STREAM_CHUNKS.length,
				text,
			});
			await sleep(25);
			if (!enabled || generation !== streamGeneration) return { action: "handled" };
		}
